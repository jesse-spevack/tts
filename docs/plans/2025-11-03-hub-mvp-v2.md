...
**Step 5: Configure letter_opener** ✅
...
## Task 2: Choose and Add Email Service ✅
...
## Task 3: Set Up Authentication with Magic Links ✅

**Goal:** Working magic link authentication

**Status:** COMPLETED

**Implementation:** Custom passwordless magic link authentication with bcrypt token hashing

**What was built:**

1. **Database Schema:**
   - `users` table: `email_address`, `auth_token` (hashed), `auth_token_expires_at`
   - `sessions` table: `user_id`, `ip_address`, `user_agent`
   - Migrations created and run ✅

2. **Service Layer (6 services):**
   - `GenerateAuthToken.call(user:)` - Creates secure token, hashes with bcrypt, returns raw token for email
   ...
   - `VerifyHashedToken.call(hashed_token:, raw_token:)` - Compares raw token against bcrypt hash
   - `InvalidateAuthToken.call(user:)` - Clears auth token and expiration, logs invalidation
...
5. **Mailer:**
   - `SessionsMailer.magic_link(user:, token:)` - Sends magic link emails using Resend
   - Subject: "🎙️ Your TTS Login Link"
   - HTML-only template (no plain text)
   - Mailer preview uses mock user object (no database interaction)

6. **Authentication Flow (RESTful):**
   - User enters email → POST `/session` → sends magic link email
   - User clicks link → GET `/session/new?token=xxx` → authenticates → redirects
   - DELETE `/session` → logs out

**Testing:**
- 44 tests, 90 assertions, all passing ✅
- RuboCop clean ✅
- Comprehensive service tests for all 6 services
- Test environment variables configured (MAILER_FROM_ADDRESS, MAILER_HOST)

**Security Features:**
- ✅ Tokens hashed with bcrypt before database storage
- ✅ One-time use tokens (invalidated after successful auth)
- ✅ 30-minute token expiration
- ✅ Constant-time comparison to prevent timing attacks
- ✅ Audit logging on token invalidation

**Key Decisions:**
- ✅ Bcrypt for token hashing (database breach doesn't expose usable tokens)
- ✅ Service objects for all business logic (not in models/controllers)
- ✅ Named parameters for mailer (user:, token:)
- ✅ RESTful routes only (no custom authenticate action)
- ✅ No fallback environment variable defaults in production (fail-fast)
- ✅ Follows gournal app authentication pattern
...
- Add bcrypt gem for token hashing
- Implement token hashing with VerifyHashedToken service
- Extract InvalidateAuthToken service with logging
- Remove mailer config fallbacks for fail-fast behavior
- Fix mailer preview to use mock user object
...
## Task 5: Auto-Create Podcast on User Signup ✅
...
**Status:** COMPLETED - Episodes controller with upload form implemented. All tests passing. Data migration added to backfill podcasts for existing users.

**Step 1: Generate Episodes controller** ✅
...
**Status:** COMPLETED - Google Cloud gems added and configured.
...
**Status:** COMPLETED - Episode processing flow implemented with GcsUploader and CloudTasksEnqueuer services.

**Goal:** Upload markdown to GCS, enqueue Cloud Task to Generator

**What was built:**

1. **GcsUploader service** (`hub/app/services/gcs_uploader.rb`):
   - Uploads episode markdown to GCS staging area
   - Uses Google Cloud Storage SDK

2. **CloudTasksEnqueuer service** (`hub/app/services/cloud_tasks_enqueuer.rb`):
   - Enqueues episode processing tasks to Google Cloud Tasks
   - Sends podcast_id, episode_id, staging_path, and metadata to Generator

3. **EpisodeSubmissionService** (`hub/app/services/episode_submission_service.rb`):
   - Orchestrates the upload and enqueue flow
   - Called from EpisodesController#create

4. **Episodes controller integration**:
   - Creates episode record
   - Uploads markdown to GCS staging
   - Enqueues Cloud Task for Generator processing

**Commits:**
- feat: Episode processing flow with Hub-Generator integration (Tasks 7-8) (#9)

---

## Task 9: Add Generator Callback Endpoint

**Goal:** RESTful endpoint for Generator to update episode status via PATCH

**Step 1: Create directory structure**

```bash
mkdir -p hub/app/controllers/api/internal
mkdir -p hub/test/controllers/api/internal
```

**Step 2: Write controller tests first**

Create `hub/test/controllers/api/internal/episodes_controller_test.rb`:
```ruby
require "test_helper"
...
**Step 4: Create Internal API controller**

Create `hub/app/controllers/api/internal/episodes_controller.rb`:
```ruby
module Api
  module Internal
    class EpisodesController < ApplicationController
...
```bash
cd hub && rails test test/controllers/api/internal/episodes_controller_test.rb
```
...
```bash
curl -X PATCH http://localhost:3000/api/internal/episodes/1 \
  -H "X-Generator-Secret: your-secret" \
  -H "Content-Type: application/json" \
  -d '{"status":"complete","gcs_episode_id":"episode_abc","audio_size_bytes":1000000}'
```
...
```bash
git add hub/
git commit -m "feat(hub): add RESTful Generator callback endpoint"
```

---

## Task 10: Update Generator HubCallbackClient for RESTful Endpoint

**Goal:** Update HubCallbackClient to use PATCH instead of POST

**Step 1: Write tests for HubCallbackClient**

Create `test/test_hub_callback_client.rb`:
```ruby
...
class TestHubCallbackClient < Minitest::Test
  ...
  def test_notify_complete_uses_patch_method
    stub = stub_request(:patch, "#{@hub_url}/api/internal/episodes/123")
      .with(
        headers: {
          "Content-Type" => "application/json",
          "X-Generator-Secret" => @secret
    ...
    episode_data = { "id" => "episode_abc", "file_size_bytes" => 1_000_000 }
    @client.notify_complete(episode_id: 123, episode_data: episode_data)

    assert_requested stub
  end

  def test_notify_failed_uses_patch_method
    stub = stub_request(:patch, "#{@hub_url}/api/internal/episodes/456")
      .with(
        headers: {
          "Content-Type" => "application/json",
          "X-Generator-Secret" => @secret
    ...
    @client.notify_failed(episode_id: 456, error_message: "TTS service unavailable")

    assert_requested stub
  end

  def test_uses_correct_restful_path
    stub = stub_request(:patch, "#{@hub_url}/api/internal/episodes/789")
      .to_return(status: 200)

    episode_data = { "id" => "ep_xyz", "file_size_bytes" => 500_000 }
    @client.notify_complete(episode_id: 789, episode_data: episode_data)

    # Verify it's NOT using old paths like /episodes/789/complete
    assert_not_requested :patch, "#{@hub_url}/api/internal/episodes/789/complete"
    assert_requested stub
  end
end
```

**Step 2: Update HubCallbackClient**

Edit `lib/hub_callback_client.rb`:
```ruby
require "net/http"
require "uri"
require "json"
...
```bash
rake test TEST=test/test_hub_callback_client.rb
```

Expected: All tests pass

**Step 4: Verify all Generator tests pass**

```bash
rake test
```

Expected: All tests pass including existing callback integration tests

**Step 5: Commit**

```bash
git add lib/hub_callback_client.rb test/test_hub_callback_client.rb
git commit -m "refactor: update HubCallbackClient to use RESTful PATCH endpoint"
```
...
- [x] User automatically gets a podcast
- [x] User can upload markdown episode
- [x] Episode uploads to GCS staging
- [x] Cloud Task enqueued to Generator
- [x] Generator processes episode
- [x] Generator calls back to Hub
- [x] Episode status updates to complete
- [x] RSS feed URL displayed
- [x] All tests pass
