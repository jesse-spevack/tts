...
**Step 7: Update Kamal secrets**

Edit `.kamal/secrets`, add:
```bash
RESEND_API_KEY=$(kamal secrets fetch RESEND_API_KEY)
```
...
Expected: Creates User model, SessionsController, PasswordsController, PasswordsMailer
...
**Step 3: Update mailer to use Resend**

Edit `app/mailers/passwords_mailer.rb`:
```ruby
class PasswordsMailer < ApplicationMailer
  def reset
    @user = params[:user]
    @signed_id = @user.signed_id(purpose: :password_reset, expires_in: 30.minutes)

    # Use Resend in production, letter_opener in development
    if Rails.env.production?
      Resend::Emails.send({
        from: 'auth@yourdomain.com',
        to: @user.email,
        subject: 'Reset your password',
        html: render_to_string(template: 'passwords_mailer/reset')
      })
    else
...
Visit `http://localhost:3000`, sign up with email, check letter_opener for magic link.

Expected: Magic link authentication works

**Step 5: Commit**

```bash
git add hub/
git commit -m "feat(hub): add magic link authentication"
```
