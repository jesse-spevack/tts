...
class FetchesUrl
  ...
  def call
    unless valid_url?
      log_warn "url_validation_failed", url: url
      return Result.failure("Invalid URL")
    end

    unless safe_host?
      log_warn "url_blocked_internal", url: url
    ...
    if content_length && content_length > AppConfig::Content::MAX_FETCH_BYTES
      log_warn "url_fetch_too_large", url: url, content_length: content_length, max: AppConfig::Content::MAX_FETCH_BYTES
      return Result.failure("Content too large")
    end

    log_info "url_fetch_request", url: url
    response = connection.get(url)

    unless response.success?
      log_warn "url_fetch_http_error", url: url, status: response.status
    ...
    if response.body.bytesize > AppConfig::Content::MAX_FETCH_BYTES
      log_warn "url_fetch_body_too_large", url: url, bytes: response.body.bytesize
      return Result.failure("Content too large")
    end

    log_info "url_fetch_success", url: url, status: response.status, bytes: response.body.bytesize
    Result.success(response.body)
  rescue Faraday::TimeoutError
    log_warn "url_fetch_timeout", url: url
    Result.failure("Could not fetch URL")
  rescue Faraday::ConnectionFailed => e
    log_warn "url_fetch_connection_failed", url: url, error: e.message
    Result.failure("Could not fetch URL")
  end
  ...
  def validate_redirect_target(_old_env, new_env)
    ...
    if addresses.empty? || addresses.any? { |addr| blocked_ip?(addr) }
      log_warn "redirect_blocked_internal", url: new_url
      raise Faraday::ConnectionFailed, "Redirect to blocked address"
    end
  rescue Resolv::ResolvError, Timeout::Error
    log_warn "redirect_dns_failed", url: new_url
