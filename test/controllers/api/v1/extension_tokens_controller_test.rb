  ...
  module V1
    class ExtensionTokensControllerTest < ActionDispatch::IntegrationTest
      ...
      test "create returns token when logged in" do
        ...
        assert json["token"].start_with?("tts_ext_")
        assert json["prefix"].present?
      end
      ...
      test "create revokes previous tokens for user" do
        ...
        first_api_token = ApiToken.find_by_token(first_token)
        ...
        second_api_token = ApiToken.find_by_token(second_token)
