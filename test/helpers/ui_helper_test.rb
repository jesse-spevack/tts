...
class UiHelperTest < ActionView::TestCase
  ...
  # the user is premium (complimentary or unlimited). Free and credit-only
  # users do not see the Billing section.

  test "show_billing_section? returns true when user is premium" do
    user = Struct.new(:premium?).new(true)
    assert show_billing_section?(user)
  end

  test "show_billing_section? returns false when user is not premium" do
    user = Struct.new(:premium?).new(false)
    assert_not show_billing_section?(user)
  end
